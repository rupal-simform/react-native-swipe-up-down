...
export default class SwipeUpDown extends Component<Props> {
  ...
  _onPanResponderMove(event, gestureState) {
    if (gestureState.dy > 0 && !this.checkCollapsed) {
      ...
      this.customStyle.style.bottom = this.props.bottomMargin;
  ...
  showMini() {
    const { onShowMini, itemMini, isTabsVisible, bottomMargin } = this.props;
    if (itemMini) {
      if (isTabsVisible) {
        this.customStyle.style.top = DEVICE_HEIGHT - this.SWIPE_HEIGHT - bottomMargin;
      } else {
        this.customStyle.style.top = DEVICE_HEIGHT - this.SWIPE_HEIGHT;
      }
    } else {
      this.customStyle.style.top = DEVICE_HEIGHT;
    }
    this.customStyle.style.height = itemMini ? this.SWIPE_HEIGHT : 0;
    this.customStyle.style.bottom = itemMini ? bottomMargin : 0;
